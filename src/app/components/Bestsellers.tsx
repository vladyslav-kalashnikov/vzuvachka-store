...
export function Bestsellers() {
    ...
    useEffect(() => {
        async function fetchBestsellers() {
            ...
            if (data) {
                setBestsellers(data.map(dbItem => ({
                    id: dbItem.id,
                    slug: dbItem.slug,
                    name: dbItem.name,
                    type: dbItem.type,
    ...
    const renderCard = (product: Product) => {
        ...
        return (
            <article className="tech-clip group relative overflow-hidden border border-white/10 bg-[#111] p-4 transition-all hover:-translate-y-1 hover:border-white/30 hover:shadow-[0_30px_80px_rgba(0,0,0,0.35)]">
                ...
                <div>
                    <p className="mb-1 text-[10px] font-bold uppercase tracking-widest text-gray-500">{product.type}</p>
                    <a href={`#product/${product.slug}`}>
                        <h3 className="mb-4 text-xl font-black uppercase leading-tight tracking-tight text-white hover:text-red-500 sm:text-lg">{product.name}</h3>
                    </a>
                    ...
                    <div className="flex flex-col items-start gap-3 border-t border-white/10 pt-4 sm:flex-row sm:items-center sm:justify-between">
                        <span className="font-mono font-bold text-white">від {formatPrice(profile.priceTiers[0].unitPrice)} / {profile.unitLabel}</span>
                        <a href={`#product/${product.slug}`} className="border border-white/10 px-4 py-2 text-[10px] font-black uppercase tracking-[0.16em] text-white transition hover:border-red-500 hover:text-red-500">В заявку</a>
                    </div>
                </div>
            </article>
        );
                    ...
                    <div className="-mx-4 overflow-x-auto px-4 pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                        <div className="flex snap-x snap-mandatory gap-4">
                            {bestsellers.map((product) => (
                                <div key={product.id} className="w-[82vw] min-w-[82vw] max-w-[340px] shrink-0 snap-start">
                                    {renderCard(product)}
                                </div>
                            ))}
                        </div>
